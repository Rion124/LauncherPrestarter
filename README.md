...
### Удалённый конфиг: сборка exe один раз

Чтобы не пересобирать exe при каждом изменении, вшивайте не адрес jar-а, а адрес
**манифеста**:

```bash
PRESTARTER_CONFIG_URL=https://host/prestarter.json PRESTARTER_STORE_DIR=Alterra   yarn tauri build
```

`prestarter.json` на сервере:

```json
{
  "launcherUrl": "https://host/ls/Launcher.jar",
  "launcherSha256": ""
}
```

Теперь путь к лаунчеру (и, при желании, жёсткий хэш) меняется правкой JSON на
сервере — раздавать игрокам новый exe не нужно. Можно указать несколько адресов
манифеста через запятую: они пробуются по порядку, то есть второй хост работает
как резервный. Последний удачно полученный манифест кэшируется в
`<папка хранения>/prestarter-remote.json` и используется, когда сервер недоступен.

Приоритет источников: `PRESTARTER_LAUNCHER_SHA256` (вшит) → `launcherSha256` из
манифеста → `<launcherUrl>.sha256` рядом с jar-ом. Адрес: `launcherUrl` из
манифеста → вшитый `PRESTARTER_LAUNCHER_URL`.

Папка хранения (`PRESTARTER_STORE_DIR`) остаётся вшитой: она нужна раньше, чем
появляется сеть — в ней лежит кэш манифеста.

| Переменная | Что делает |
|---|---|
| `PRESTARTER_CONFIG_URL` | Адрес манифеста (можно несколько через запятую). Единственное, что стоит вшивать |
